...
export class VpcNetworkContextProviderPlugin implements ContextProviderPlugin {
  ...
  private async findVpc(ec2: EC2Client, args: cxschema.VpcContextQuery): Promise<Vpc> {
    ...
    const command = new DescribeVpcsCommand({ Filters: filters });
  ...
  private async readVpcProps(
    ec2: EC2Client,
    vpc: Vpc,
    args: cxschema.VpcContextQuery,
  ): Promise<cxapi.VpcContextResponse> {
    ...
    const describeSubnetsCommand = new DescribeSubnetsCommand(filters);
    const subnetsResponse = await ec2.send(describeSubnetsCommand);
    const listedSubnets = subnetsResponse.Subnets || [];

    const describeRouteTablesCommand = new DescribeRouteTablesCommand(filters);
    const routeTablesResponse = await ec2.send(describeRouteTablesCommand);
    ...
    const vpnGatewayResponse = (args.returnVpnGateways ?? true) ? await ec2.send(command) : undefined;
    const vpnGatewayId =
...
function subnetTypeToVpcSubnetType(type: SubnetType): cxapi.VpcSubnetGroupType {
  switch (type) {
    case SubnetType.Isolated:
      return cxapi.VpcSubnetGroupType.ISOLATED;
    case SubnetType.Private:
      return cxapi.VpcSubnetGroupType.PRIVATE;
    case SubnetType.Public:
      return cxapi.VpcSubnetGroupType.PUBLIC;
  }
}
